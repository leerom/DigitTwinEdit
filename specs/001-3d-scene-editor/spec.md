...
- **FR-001**: 系统必须提供一个 3D 视口，实时渲染场景对象、网格、灯光和相机。
- **FR-002**: 系统必须支持 3D 透视视图和 2D 正交视图模式。
- **FR-003**: 系统必须在右下角显示坐标轴辅助工具，显示 X(红)/Y(绿)/Z(蓝) 方向。
- **FR-004**: 系统必须在视口叠加层中显示性能统计信息（绘制调用、三角形、顶点）。
- **FR-059**: 系统必须支持通过浏览器文件下载将当前场景保存为 JSON 文件。

#### 导航

- **FR-005**: 系统必须支持通过 Alt+左键拖动旋转视图（仅 3D 模式）。
- **FR-006**: 系统必须支持通过鼠标滚轮或 Alt+右键拖动缩放视图。
- **FR-007**: 系统必须支持通过中键拖动或 Q 工具+左键拖动平移视图。
- **FR-008**: 系统必须支持在按住右键的同时进行飞行漫游导航（WASD/QE 移动）（仅 3D 模式）。
- **FR-009**: 系统必须更改光标图标以反映当前模式：箭头（默认）、手形（平移）、眼睛（飞行漫游）。
- **FR-010**: 系统必须在按住 Shift 键时加速所有导航操作。
- **FR-011**: 系统必须支持方向键导航以平移视图。

#### 选择

- **FR-012**: 系统必须支持通过左键单击进行单对象选择。
- **FR-013**: 系统必须支持通过左键拖动进行框选。
- **FR-014**: 系统必须支持通过 Ctrl+单击进行加选。
- **FR-015**: 系统必须支持通过 Alt+单击进行减选。
- **FR-016**: 系统必须支持通过 Ctrl+A 全选。
- **FR-017**: 系统必须在场景视图和层级面板之间双向同步选择状态。

#### 变换工具

- **FR-018**: 系统必须提供抓手工具 (Q) 用于仅视图导航。
- **FR-019**: 系统必须提供移动工具 (W)，显示 XYZ 轴箭头用于位置编辑。
- **FR-020**: 系统必须提供旋转工具 (E)，显示 XYZ 轴环用于旋转编辑。
- **FR-021**: 系统必须提供缩放工具 (R)，显示 XYZ 轴立方体用于缩放编辑。
- **FR-022**: 系统必须提供通用工具 (Y)，组合移动/旋转/缩放控件。
- **FR-023**: 系统必须对变换 Gizmo 进行颜色编码：X 轴（红），Y 轴（绿），Z 轴（蓝）。
- **FR-024**: 系统必须支持键盘快捷键 Q/W/E/R/Y 进行工具切换。
- **FR-025**: 系统必须在按住 Shift 键时加速变换操作。
- **FR-026**: 系统必须通过视觉指示器高亮工具栏中当前活动的工具。
- **FR-027**: 系统必须在未选中对象时隐藏变换 Gizmo（抓手工具除外）。

#### 渲染模式

- **FR-028**: 系统必须提供线框渲染模式，仅显示对象几何线条。
- **FR-029**: 系统必须提供着色渲染模式，显示完整的材质和光照（默认）。
- **FR-030**: 系统必须提供线框+着色渲染模式，结合两种可视化效果。
- **FR-031**: 系统必须显示当前活动渲染模式的视觉指示器。
- **FR-032**: 系统必须强制渲染模式之间的互斥性。

#### 视图聚焦

- **FR-033**: 系统必须在按下 F 键时将视图聚焦于选中的对象。
- **FR-034**: 系统必须在按下 Shift+F 时锁定视图以跟随选中的对象。
- **FR-035**: 系统必须在 Shift+点击坐标轴中心时将视图重置为默认透视角度。

#### 层级面板

- **FR-036**: 系统必须在可折叠的树状结构中显示所有场景对象。
- **FR-037**: 系统必须支持拖放以更改对象父子关系。
- **FR-038**: 系统必须以独特的视觉样式高亮显示选中的对象。
- **FR-039**: 系统必须在对象名称旁显示对象类型图标。

#### 检查器面板

- **FR-040**: 系统必须为选中对象显示变换组件（位置、旋转、缩放）。
- **FR-041**: 系统必须允许对所有变换值进行带有 XYZ 标签的数字输入。
- **FR-042**: 系统必须为选中对象显示带有视觉滑块的材质属性。
- **FR-043**: 系统必须显示数字孪生数据部分，包括外部 ID、实时值和连接状态。
- **FR-044**: 系统必须在对象变换时实时更新检查器值。
- **FR-045**: 系统必须允许通过 F2 或直接名称输入重命名对象。

#### 项目面板

- **FR-046**: 系统必须提供项目标签页，显示项目资产文件夹树（从静态本地文件加载）。
- **FR-047**: 系统必须提供资源标签页，显示公共资产库。
- **FR-048**: 系统必须在内容区域显示带有文件类型指示器的资产缩略图。
- **FR-049**: 系统必须支持从项目面板拖放到场景视图以进行实例化。

#### 对象操作

- **FR-050**: 系统必须支持通过 Ctrl+D 复制对象。
- **FR-051**: 系统必须支持通过 Delete 键删除对象（需确认）。
- **FR-052**: 系统必须支持通过 Shift+Delete 立即删除对象（无确认）。
- **FR-053**: 系统必须支持通过 Ctrl+Shift+Z 重置变换。
- **FR-054**: 系统必须支持通过 Ctrl+Z 撤销。
- **FR-055**: 系统必须支持通过 Ctrl+Y 重做。

#### 跨平台

- **FR-056**: 系统必须适配 Mac 的键盘快捷键（Ctrl→Command，Alt→Option）。
- **FR-057**: 系统必须支持带有响应式布局的窗口调整大小。
- **FR-058**: 系统必须通过禁用视图旋转和飞行漫游导航来限制 2D 模式。

#### 技术栈 (Technical Constraints)

- **TC-001**: 必须使用 **React + Three.js (React Three Fiber)** 作为前端框架和 3D 渲染引擎。
- **TC-002**: 必须使用 **Zustand** (轻量级状态管理) 来同步编辑器状态（替代 Redux/Context）。
- **TC-003**: 必须使用 **Instanced Mesh Rendering** 技术来处理相同几何体/材质的对象，以确保支持 1000+ 对象时的 60fps 性能。
- **TC-004**: 必须使用 **Tailwind CSS** 作为 UI 样式框架，以匹配提供的设计样本。
...
- **SC-001**: 用户可以在每次交互 100 毫秒响应时间内导航任何场景视图（旋转、平移、缩放）。
- **SC-002**: 用户可以通过单击选择对象，并在 100 毫秒内在层级中看到同步高亮显示。
- **SC-003**: 用户可以使用键盘快捷键在所有 5 个变换工具 (Q/W/E/R/Y) 之间切换，无明显延迟。
- **SC-004**: 场景在最多可见 1000 个对象的情况下以最低 60fps 渲染。
- **SC-005**: 用户可以在 30 秒内完成基本的对象放置任务（选择工具、定位对象、确认）。
- **SC-006**: 所有视觉反馈（工具激活、选择高亮、光标更改）在用户操作后 100 毫秒内出现。
- **SC-007**: 变换 Gizmo 正确显示与坐标系方向匹配的轴颜色（红-X，绿-Y，蓝-Z）。
- **SC-008**: 用户可以撤销/重做至少 50 个连续操作而不丢失数据。
- **SC-009**: 当用户拖动对象时，检查器面板实时更新变换值，无明显延迟。
- **SC-010**: 90% 的首次用户可以在不查阅文档的情况下成功导航、选择和变换对象。
