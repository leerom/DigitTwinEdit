...
## Task 3: NodeLibraryPanel 全面重设计 {#task-3}

**文件:**
- Modify: `packages/client/src/features/nodeMaterial/NodeLibraryPanel.tsx`

**目标:** bg-panel-dark、NODES 标题区、分类标题左色条、节点项彩色圆点 hover 效果
...
```bash
pnpm --filter client test -- --run src/features/nodeMaterial/NodeLibraryPanel.test.tsx 2>&1 | tail -8
```

Expected: 3 tests PASS（测试通过，data-testid 保持不变）

### Step 3: Commit

```bash
git add packages/client/src/features/nodeMaterial/NodeLibraryPanel.tsx
git commit -m "style(nodeMaterial): redesign NodeLibraryPanel - panel-dark bg, color strips, dot indicators"
```
